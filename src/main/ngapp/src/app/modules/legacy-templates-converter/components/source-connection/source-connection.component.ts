...
export class SourceConnectionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.source_use_authentication_enabled =
      this.projectFormService.source_use_authentication_enabled;
  ...
  onTestSource() {
    const connection_settings = this.projectForm.get(
      'connection_settings'
    ).value;
    this.apiService
      .testTemplatesConverter(this.projectId.value, connection_settings)
      .subscribe({
